---
tags:
  - evocation
  - Dragon-Blooded
Cost: 4m
Mins: Essence 2
Type: Reflexive
Keywords:
  - Mute
  - Resonant
Duration: Instant
Prerequisite Charms:
  - "[[ Clairvoyant Mirror Technique]]"
---
Perdurant Vault shelters the wielder’s psyche behind an ephemeral spiritual wall. The wielder gains +2 Resolve against an influence roll, or negates the Guile penalty for being unaware of an observer on a single roll. 

## Resonant

Each time a character resonant with jade uses this Evocation, the cost of subsequent activations in the same scene is cumulatively discounted by one mote, to a minimum of zero.